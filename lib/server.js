...
function Server(options) {
    ...
    assert.optionalString(options.engine, 'options.engine');
    ...
    if (options.engine === 'uws') {
        // POC: uWebSockets.js engine. Lazy-require so a missing optional
        // native module never breaks the default Node http path.
        // eslint-disable-next-line global-require
        var uwsAdapter = require('./uwsAdapter');

        patchResponse(uwsAdapter.UwsResponse);
        patchRequest(uwsAdapter.UwsRequest);

        this.uws = true;
        this.server = uwsAdapter.createServer({});
    } else if (options.spdy) {
    ...
    if (!options.handleUpgrades && !this.uws) {
    ...
    // Now the things we can't blindly proxy. Skipped under the uws engine: the
    // uWebSockets.js adapter does not surface clientError / checkContinue /
    // upgrade events (out of scope for the POC).
    if (!this.uws) {
